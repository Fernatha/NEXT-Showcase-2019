...
public:
	...
	//checks the see if spaces within 2 tiles of current tile is a powerpill
	bool CheckNeighbour(const int x, const int y);
	//These are used for the pill count
	void SetPillCount(int p);
	int GetPillCount();
private:
    // Create a new map.
    void Create();
    ...
    float m_tileWidth;
    float m_tileHeight;
	int m_pillCount;								//used to determine how many pills there are and when there are none left
